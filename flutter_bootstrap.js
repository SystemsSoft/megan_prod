// Bootstrap customizado do Flutter — idêntico ao padrão gerado a cada build,
// menos uma coisa: NÃO passamos `serviceWorkerSettings` para o loader.
//
// O padrão faz `_flutter.loader.load({serviceWorkerSettings: {...}})`, e isso
// registra o `flutter_service_worker.js` no escopo raiz. Só que esse arquivo,
// desde que o Flutter descontinuou o service worker dele
// (https://github.com/flutter/flutter/issues/156910), virou uma lápide de
// migração: o handler de `activate` dele chama `self.registration.unregister()`
// e se apaga sozinho. Como ele ocupa o MESMO escopo ("/") de qualquer service
// worker nosso, deixá-lo rodar apagava o nosso junto — o site ficava
// permanentemente sem service worker nenhum.
//
// Sem service worker com handler de `fetch`, o Chrome não considera o site
// instalável: o evento `beforeinstallprompt` nunca dispara e a opção
// "Instalar app" não aparece nem no menu do navegador. Quem registra o nosso
// service worker de verdade é o web/index.html (ver pwa_service_worker.js).
//
// Os dois placeholders abaixo são substituídos pelo build do Flutter — ver
// generateDefaultFlutterBootstrapScript em flutter_tools/lib/src/web/bootstrap.dart.
